...
import type {
  ...
  DAGEdge,
  CEdge,
} from "@/types/zcashd-deprecation.types";
...
const nodes: Record<IDS, DAGNode> = {
  [IDS.I_1363]: {
    description:
      "Enumerate the capabilities exposed by zcashd APIs for spending transparent coins.",
    status: "closed",
  },
  [IDS.I_1350]: {
    description:
      "Determine what subset of zcashd wallet functionality will be required by users in a replacement wallet (independent of how it is accessed)",
    status: "workingon",
  },
  [IDS.I_1412]: {
    description:
  ...
  [IDS.I_1373]: {
    description: "Agree on a sketch of the overall wallet architecture",
  },
  ...
  [IDS.I_1395]: {
    description: "Enumerate the data that the indexer service needs to provide",
  },
  [IDS.I_1367]: {
    description:
  ...
  [IDS.I_4099]: {
    description: "Investigate the distribution of P2SH scripts",
    repo: "zcash",
  },
  [IDS.I_1372]: {
    description:
  ...
  [IDS.I_1366]: {
    description:
      "Enable wallets build with the `zcash_client_backend` APIs to track multiple candidate chain tips",
  },
  [IDS.I_1351]: {
    description:
      "Wrap the `zcash_client_backend` sync engine as a stateful wallet service application.",
  },
  [IDS.I_1381]: {
    description:
  ...
  [IDS.I_1362]: {
    description:
      "Create a generator and evaluator for arbitrary transparent Zcash script.",
  },
  [IDS.I_1379]: {
    description:
      "`zcash_client_backend`: Add support for full P2PKH history tracking",
  },
  [IDS.I_578]: {
    description:
      "Survey key storage and usage of `zcashd` and `zcash_client_sqlite`",
  },
  [IDS.I_1188]: {
    description:
  ...
  [IDS.I_675]: {
    description: "[ZIP 332] Wallet Recovery from zcashd HD Seeds",
    repo: "zips",
  },
  [IDS.I_6873]: {
    description:
  ...
  [IDS.I_1364]: {
    description:
      "Consider whether we provide drop-in support for at least some `zcashd` RPC APIs.",
  },
  [IDS.I_1360]: {
    description:
      "Transaction builder support for spending (almost) arbitrary transparent inputs.",
  },
  [IDS.I_1371]: {
    description:
      "Create structures for storing broader kinds of transparent spending key material",
  },
  [IDS.I_1361]: {
    description:
      "`zcash_client_backend`: Add support for full transparent coin tracking (P2PKH and P2SH)",
  },
  [IDS.I_1348]: {
    description:
      "zcash_client_backend: Add the ability to restore from `zcashd` seed phrases.",
  },
  [IDS.I_5796]: {
    description:
  ...
  [IDS.I_6453]: {
    description:
      "Provide a wallet upgrade path that moves funds held by legacy keys into a unified account.",
    repo: "zcash",
  },
  [IDS.I_821]: {
    description: "Define a standard wallet export format.",
    repo: "zips",
  },
  [IDS.I_1411]: {
    description:
  ...
  [IDS.I_1369]: {
    description:
      "Implement the subset of `zcashd` RPCs that expose the decided-upon wallet functionality",
  },
  [IDS.I_1370]: {
    description:
  ...
  [IDS.I_1353]: {
    description:
      "zcash_client_backend: If a seed is reused, transparent UTXO spends from one wallet may not be detected in the other.",
  },
  [IDS.I_579]: {
    description: "Redesign Rust types for representing wallet key material",
  },
  [IDS.I_1365]: {
    description:
      "Add the functionality to restore from `zcashd` wallet.dat or wallet dump files.",
  },
  [IDS.I_1074]: {
    description:
      "Transparent input details should be retrieved with transactions on account recovery",
  },
  [IDS.I_1349]: {
    description:
      "zcash_client_backend: Required functionality for full `zcashd` wallet replacement.",
  },
};
...
const enrichedEdges: Edge[] = edges.map((edge) => ({
